...
{
    esp_err_t ret_val = ESP_ERR_NOT_SUPPORTED;
    ...
    if (codec_type & (1 << CODEC_TYPE_ES8311)) {
        ret_val = bsp_codec_dac_init(dac_sample_rate);
    }

    if (codec_type & (1 << CODEC_TYPE_ES7210)) {
        ret_val = bsp_codec_adc_init(adc_sample_rate);
    }

    if (ret_val != ESP_OK) {
        ESP_LOGW(TAG, "Currently not support, ret_val: %d", ret_val);
    }
    return ret_val;
}
